...
export const BottomNav: React.FC<BottomNavProps> = ({ activeTab, onTabChange }) => {
  ...
  return (
    <div className="fixed bottom-0 left-0 w-full z-40 pointer-events-none flex justify-center md:pb-5">
      <nav className="pointer-events-auto w-full md:max-w-lg md:rounded-2xl bg-white/95 backdrop-blur-xl border-t md:border border-slate-200/80 md:shadow-[0_12px_35px_rgba(0,0,0,0.1)] shadow-[0_-5px_20px_rgba(0,0,0,0.05)] pb-[env(safe-area-inset-bottom)] transition-all">
        <div className="flex justify-around items-center h-16 md:h-18 px-2 md:px-4">
          {navItems.map((item) => {
              ...
              >
                <div className="relative">
                  ...
                  <Icon
                    ...
                    size={24}
                    filled={isActive}
                    className={`transition-all duration-300 ${!isActive ? 'group-hover:scale-110' : ''} ${isAdvisor ? 'animate-[ai-glow_2s_infinite_alternate]' : ''}`}
                  />
                </div>
                ...
                {/* Underlying glow for Advisor */}
                {isAdvisor && !isActive && (
                  <div className="absolute bottom-2 w-8 h-1 bg-primary/20 blur-md animate-pulse"></div>
                )}
              </button>
            );
          })}
